...
class C_provider extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('m_provider'); // Load model SiswaModel.php yang ada di folder models
	...
	public function index(){
		// $this->load->view('v_pasien');
         ...
         $this->load->view('v_provider',$data);
	}
	public function dataedit() {
        $respond= new stdClass(); 
        $id     = $this->input->post('id_provider', true);
        $respond= $this->m_provider->findedit($id);
    ...
    function delete(){
        ...
        $sql    = $this->m_provider->deletedata($id);
	 ...
	 function view() {
        ...
        $aColumns = array('id_provider','code','name' );
        ...
        $iTotalRecords = $this->m_provider->get_total($aColumns, $sWhere);
        ...
        $data = $this->m_provider->get_data($sLimit, $sWhere, $sOrder, $aColumns);
        // var_dump($data);
        $no = 1 + $iDisplayStart;
        $ino = 0+ $iDisplayStart;
        foreach ($data as $row) {
	 ...
	 public function save() {
        ...
        $act = $this->input->post('act', true);
                ...
                if ($act == 'add') {
                 ...
                 $data['id_provider'] = $this->input->post('id_provider', true);
                 $sql = $this->m_provider->edit($data);
                    if ($sql == true) {
                        echo "1up";
                    } else {
                        echo "0up";
