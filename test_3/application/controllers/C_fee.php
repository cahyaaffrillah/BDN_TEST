...
class C_fee extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('m_fee'); // Load model SiswaModel.php yang ada di folder models
	...
	public function index(){
		// $this->load->view('v_pasien');
         ...
         $this->load->view('v_fee',$data);
	}
	public function dataedit() {
        $respond= new stdClass(); 
        $id     = $this->input->post('id_fee', true);
        $respond= $this->m_fee->findedit($id);
    ...
    function delete(){
	 ...
	 function view() {
        ...
        $aColumns = array('a.id_fee','b.code as code_channel','c.code as code_provider','a.admin_fee' );
        ...
        $iTotalRecords = $this->m_fee->get_total($aColumns, $sWhere);
        ...
        $data = $this->m_fee->get_data($sLimit, $sWhere, $sOrder, $aColumns);
        // var_dump($data);
        $no = 1 + $iDisplayStart;
        $ino = 0+ $iDisplayStart;
        foreach ($data as $row) {
	 ...
	 public function save() {
        ...
        $act = $this->input->post('act', true);
                ...
                if ($act == 'add') {
                 ...
                 $data['id_fee'] = $this->input->post('id_fee', true);
                 $sql = $this->m_fee->edit($data);
                    if ($sql == true) {
                        echo "1up";
                    } else {
                        echo "0up";
