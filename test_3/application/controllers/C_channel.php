...
class C_channel extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('m_channel'); // Load model SiswaModel.php yang ada di folder models
	...
	public function index(){
		// $this->load->view('v_pasien');
         ...
         $this->load->view('v_channel',$data);
	}
	public function dataedit() {
        $respond= new stdClass(); 
        $id     = $this->input->post('id_channel', true);
        $respond= $this->m_channel->findedit($id);
    ...
    function delete(){
        ...
        $sql    = $this->m_channel->deletedata($id);
	 ...
	 function view() {
        ...
        $aColumns = array('id_channel','code','name' );
        ...
        $iTotalRecords = $this->m_channel->get_total($aColumns, $sWhere);
        ...
        $data = $this->m_channel->get_data($sLimit, $sWhere, $sOrder, $aColumns);
        // var_dump($data);
        $no = 1 + $iDisplayStart;
        $ino = 0+ $iDisplayStart;
        foreach ($data as $row) {
	 ...
	 public function save() {
        ...
        $act = $this->input->post('act', true);
                ...
                if ($act == 'add') {
                 ...
                 $data['id_channel'] = $this->input->post('id_channel', true);
                 $sql = $this->m_channel->edit($data);
                    if ($sql == true) {
                        echo "1up";
                    } else {
                        echo "0up";
